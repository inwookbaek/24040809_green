...
CREATE TABLE MYSCHEMA.DEPAETMENTS (
	DEPT_ID NUMBER(4) NOT NULL,
	DEPT_NAME VARCHAR2(30) NOT NULL,
	DEPT_LOC VARCHAR2(30)
);

COMMENT ON TABLE MYSCHEMA.DEPAETMENTS IS '부서테이블';

COMMENT ON COLUMN MYSCHEMA.DEPAETMENTS.DEPT_ID IS '부서번호';

COMMENT ON COLUMN MYSCHEMA.DEPAETMENTS.DEPT_NAME IS '부서이름';

COMMENT ON COLUMN MYSCHEMA.DEPAETMENTS.DEPT_LOC IS '부서위치';

CREATE UNIQUE INDEX MYSCHEMA.PK_DEPAETMENTS
	ON MYSCHEMA.DEPAETMENTS (
		DEPT_ID ASC
	);

ALTER TABLE MYSCHEMA.DEPAETMENTS
	ADD
		CONSTRAINT PK_DEPAETMENTS
		PRIMARY KEY (
			DEPT_ID
		);
...
ALTER TABLE MYSCHEMA.EMPLOYEES
	ADD
		CONSTRAINT FK_DEPAETMENTS_TO_EMPLOYEES
		FOREIGN KEY (
			DEPT_ID
		)
		REFERENCES MYSCHEMA.DEPAETMENTS (
			DEPT_ID
		);
